  ...
  return (
    <div className="flex items-center justify-between bg-gray-800/50 rounded-lg px-8 py-6">
      ...
      <div className="flex items-center">
        ...
        <h1 className="ml-4 text-3xl font-bold bg-gradient-to-r from-yellow-500 via-yellow-300 to-yellow-500 text-transparent bg-clip-text">
          BTC Beats
        </h1>
      </div>
        ...
        {currentPrice ? (
          <>
            <div className="flex items-center space-x-3">
              ...
              <span className={`text-base ${isPositive ? 'text-green-400' : 'text-red-400'}`}>
                {isPositive ? '↑' : '↓'}
                {Math.abs(priceChange || 0).toFixed(2)}%
              </span>
            </div>
            <span className="text-sm text-gray-400 mt-1">Current Price</span>
          </>
        ) : (
